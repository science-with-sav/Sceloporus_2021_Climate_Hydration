...
```{r}
CEWL_VPD <- lm(data = dat_no_rehab_deltaCEWL,
                               delta_CEWL ~ VPD_kPa_tmttrial)
anova(CEWL_VPD)
plot(CEWL_VPD)
simple.eda(residuals(CEWL_VPD))
shapiro.test(residuals(CEWL_VPD))
```
...
I want to know how the 2-day recovery period affects physiology relative to post- and pre- experiment. 

First/only calculate the mean ∆ and SEM?
...
SMI_rmod_postexp <- lmerTest::lmer(data = dat_for_rehab,
                       SMI ~  type + 
                         (1|trial_number/individual_ID))
