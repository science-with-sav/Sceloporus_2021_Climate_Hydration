...
I'm disappointed in the high variability in humidity treatment, but it is what it is.
...
humid_mod_df <- data.frame(broom::tidy(ph_humid_mod)) %>%
  arrange(desc(term, adj.p.value))
write.csv(humid_mod_df, "./results_statistics/HOBO_temp_pairwise_diffs.csv")
```
...
format_HOBO_data %>%
  ...
  geom_text(data = anova_labels_temp,
            size = 7,
            mapping = aes(x = 1, y = c(10,20,30,40), label = t1)) +
  # trial 2 labels
  geom_text(data = anova_labels_temp,
            size = 7,
            mapping = aes(x = 2, y = c(10,20,30,40), label = t2)) +
  # trial 3 labels
  geom_text(data = anova_labels_temp,
            size = 7,
            mapping = aes(x = 3, y = c(10,20,30,40), label = t3)) +
  # trial 4 labels
  geom_text(data = anova_labels_temp,
            size = 7,
            mapping = aes(x = 4, y = c(10,20,30,40), label = t4)) +
  # trial 5 labels
  geom_text(data = anova_labels_temp,
            size = 7,
            mapping = aes(x = 5, y = c(10,20,30,40), label = t5)
            ) #-> ANOVA_temp_boxplot
