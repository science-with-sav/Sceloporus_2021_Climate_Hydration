...
```{r}
# model rankings
all_top_mod_ranks <- CEWL_AICc[1:2,] %>%
  rbind(osml_AICc[1:2,]) %>% 
  dplyr::rename(LL = "Res.LL") %>%
  rbind(hct_AICc[1:2,]) %>%
  mutate(var = c(rep("CEWL", 2), rep("osml", 2), rep("hct", 2)),
...
write.csv(all_top_mod_ranks, "./results_statistics/capture_ALL_mod_rankings.csv")


# actual models
hct_SLR_format <- broom.mixed::tidy(hct_mod6) %>%
  mutate(effect = "fixed",
         group = NA,
         df = 120)
all_top_mods <- broom.mixed::tidy(CEWL_mod10p) %>%
  rbind(broom.mixed::tidy(osml_mod7p)) %>%
  rbind(hct_SLR_format) %>%
  mutate(var = c(rep("CEWL", 8), rep("osml", 6), rep("hct", 4)),
