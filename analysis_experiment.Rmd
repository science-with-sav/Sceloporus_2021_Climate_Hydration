...
Also check body temp ~ measurement temp... and CEWL ~ msmt temp

```{r}
temp_body_temp_mod <- lm(data = body_temp_test_dat, 
                         cloacal_temp_C ~ msmt_temp_C)
summary(temp_body_temp_mod)
...
CEWL_msmt_temp_mod <- lm(data = body_temp_test_dat, 
                         CEWL_g_m2h_mean ~ msmt_temp_C)
summary(CEWL_msmt_temp_mod)

HH_temp <- body_temp_test_dat %>%
  dplyr::filter(substr(tmt, 1, 6) == "Hot Hu")
CEWL_msmt_temp_HH <- lm(data = HH_temp, 
                         CEWL_g_m2h_mean ~ msmt_temp_C)
summary(CEWL_msmt_temp_HH) # p = 0.12

HD_temp <- body_temp_test_dat %>%
  dplyr::filter(substr(tmt, 1, 6) == "Hot Dr")
CEWL_msmt_temp_HD <- lm(data = HD_temp, 
                         CEWL_g_m2h_mean ~ msmt_temp_C)
summary(CEWL_msmt_temp_HD) # p = 0.02***

CH_temp <- body_temp_test_dat %>%
  dplyr::filter(substr(tmt, 1, 6) == "Cool H")
CEWL_msmt_temp_CH <- lm(data = CH_temp, 
                         CEWL_g_m2h_mean ~ msmt_temp_C)
summary(CEWL_msmt_temp_CH) # p = 0.32

CD_temp <- body_temp_test_dat %>%
  dplyr::filter(substr(tmt, 1, 6) == "Cool D")
CEWL_msmt_temp_CD <- lm(data = CD_temp, 
                         CEWL_g_m2h_mean ~ msmt_temp_C)
summary(CEWL_msmt_temp_CD) # p = 0.09
