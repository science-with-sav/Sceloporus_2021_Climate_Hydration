...
dat_no_rehab %>% 
  ...
  geom_point(data = mean_SMI,
             aes(x = day,
                 y = mean_SMI - 0.08, # slight adjustment to center on line
                 color = tmt,
                 shape = tmt),
             size = 4, 
             alpha = 1) +
...
### Cleaning
Note that we removed trial 1 data from this figure!

```{r osmolality exp fig}
# calculate means to overlay
mean_osml <- dat %>%
  dplyr::filter(day == "post-exp") %>%
  dplyr::filter(trial_number != 1) %>% 
...
dat_no_rehab %>% 
  ...
  geom_point(data = mean_osml,
             aes(x = day,
                 y = mean_osml + 5, # slight adjust to center on line 
                 color = tmt,
                 shape = tmt),
             size = 4, 
             alpha = 1) +
...
```{r osmolality exp model 1a}
osml_mod1a <- dat_no_rehab %>%
  dplyr::filter(complete.cases(osmolality_mmol_kg_mean)) %>%
  #dplyr::filter(trial_number != 1) %>%
  lme4::lmer(data = .,
...
Actually, re-run without individual ID as a randome effect, since it accounts for none of the variation in plasma osmolality:

```{r osmolality exp model 1b}
osml_mod1b <- dat_no_rehab %>%
  dplyr::filter(complete.cases(osmolality_mmol_kg_mean)) %>%
  #dplyr::filter(trial_number != 1) %>%
  lme4::lmer(data = .,
                       osmolality_mmol_kg_mean ~ 
...
We can no longer improve AIC, so this is the best model. Seemingly, temperature acclimation had no effect on plasma osmolality, but humidity had a small effect.

Alternatively, since AIC is ~same with or without the day* humidity interaction, try dropping that:
...
We do not lose predictive power. Now that the humidity effect is freed from the interaction, AIC suggests dropping that variable as well:
...
Compared to the day*humidity model (osmolality model 3), the day-only model (model 5) does not lose predictive power and has slightly better AIC, so I would consider this the best model, even though it is comparable to the model including humidity.
...
osml_rmod2 <- dat_for_rehab %>%
  ...
  lme4::lmer(data = .,
                       osmolality_mmol_kg_mean ~ 
                         type*humidity_tmt + type*temp_tmt +
                         (1|trial_number))
summary(osml_rmod2)
drop1(osml_rmod2)
```
...
Drop the interactions with day:
...
Drop the effect of rehab:

```{r osmolality rehydration model 5}
osml_rmod5 <- dat_for_rehab %>%
  dplyr::filter(complete.cases(osmolality_mmol_kg_mean)) %>%
  #dplyr::filter(trial_number != 1) %>%
  lme4::lmer(data = .,
                       osmolality_mmol_kg_mean ~ 
...
Including only temperature as a predictor seems to be the best model. HOWEVER, I would conclude that rehydration did not increase hydration at all, at least as measured by plasma osmolality. Based on AIC, the null model to explain osmolality is just as good, so I will conclude that the change in plasma osmolality was best-predicted by the null model.
...
During the 2-day rehydration opportunity provided to lizards following experimental climate treatment, body condition significantly increased and hematocrit significantly decreased. The change in body condition during rehydration was also effected by the interaction between rehydration and acclimation temperature prior to rehydration, with cool-acclimated lizards having overall higher body condition and showing a larger increase in body condition during rehydration. Hematocrit was also affected by acclimation temperature prior to rehydration, with cool-acclimated lizards maintaining higher hematocrit levels with decreases the same as hot-acclimated lizards. Nested effects of trial number and individual lizard ID were included as random effects in both LMMs. Plasma osmolality did not change during the rehydration period, and that was the same across acclimation treatments. The LMM for plasma osmolality included only trial number as a random effect.
...
I still need to determine whether trial 1 significantly effects the models for osmolality.
