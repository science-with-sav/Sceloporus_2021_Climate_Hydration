...
format_HOBO_data_w_VPD <- format_HOBO_data %>%
  mutate(e_s_kPa = 0.611 * exp((17.502*temp_C)/(temp_C + 240.97)), # saturation VP, Campbell & Normal 1998
...
tmt_trial_means <- format_HOBO_data_w_VPD %>%
  group_by(tmt, trial) %>%
  summarise(VPD_mean_tmttrial = mean(VPD_kPa),
            VPD_SD_tmttrial = sd(VPD_kPa),
            temp_mean_tmttrial = mean(temp_C),
