...
After these quick checks, these individuals still have **too few: 213, 216, 239, 245, 278, 289, 294, 303, 305 ** (all 9 observations)

```{r}
indiv_too_few <- c(213, 216, 239, 245, 278, 289, 294, 303, 305)
...
# save the ones I need to investigate further
wrong_amts <- rep_check_1 %>%
  dplyr::filter(individual_ID %in% c(indiv_too_few, indiv_too_many))
wrong_amts
```


I've already settled why there are discrepancies for individual 254. I haven't settled why individuals 213 and 216 (June 24), 245 (July 4), 278 and 289 (July 28), 294 and 305 (August 16) only have 4 observations on one date. The most likely explanation is that we miscounted replicates and only did 4, rather than 5.

Individuals 206 and 215 both have extra measurements on June 24. Individuals 237 and 302 both have **10** measurements! On July 4 and August 8, respectively.
...
rep_check_2 <- all_CEWL_data %>%
                dplyr::filter(individual_ID %in% wrong_amts$individual_ID &
                                date %in% wrong_amts$date)
```
...
### I will need to reassign the measurements attributed to individual 302 taken between 13:09-13:12 on August 8 as pre-experiment measurements for individual 303. Discrepancies in number of measurements for individuals 302 and 303 solved!
