...
The CV is >10 for only one lizard on one date, so our replicates are likely to accurately represent the true value. We don't want to lose accuracy by searching for precision, so we will only remove the one point driving the enormous CV value.
