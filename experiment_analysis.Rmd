...
CEWL_mod_best <- lmerTest::lmer(data = dat_no_rehab,
                       CEWL_g_m2h_mean ~ cloacal_temp_C + 
                         ...
                         day_n:humidity_tmt:temp_tmt + 
                         (1|trial_number/individual_ID))
length(unique(dat_no_rehab$individual_ID))
...
**CEWL and body condition were best-predicted by the FULL model. Hematocrit was best-predicted by a reduced model, with delta-AIC of the other models >3.7 (AICc_ranks / exp_model_rankings). Osmolality was predicted equally well by the full and reduced model, so we present the reduced model with the least number of variables (AICc_ranks / exp_model_rankings). The other equally-good models are in exp_best_models_osml_additional.**
...
Remove three-way interaction and see if we lose significant predictive power.
...
SMI_rmod2 <- lme4::lmer(data = dat_for_rehab,
                       SMI ~ 
                         ...
                         (1|trial_number/individual_ID))
summary(SMI_rmod2)
anova(SMI_rmod1, SMI_rmod2)
drop1(SMI_rmod2)
```

We do not lose predictive power, so we're better-off without the 3-way interaction.



try dropping either tmt effect:

```{r SMI rehydration model 2ab}
...
SMI_rmodels <- list(SMI_rmod1, 
                   SMI_rmod2, SMI_rmod2a, SMI_rmod2b, 
                   SMI_rmod3, 
                   SMI_rmod_null)

#specify model names
REHYD_mod_names <- c('(model 1) ~ Rhyd, Rhyd:Humd, Rhyd:Temp, Rhyd:Humd:Temp',
                   '(model 2) ~ Rhyd, Rhyd:Humd, Rhyd:Temp',
                   '(model 2a) ~ Rhyd, Rhyd:Temp',
                   '(model 2b) ~ Rhyd, Rhyd:Humd',
                   '(model 3) ~ Rhyd',
                   'null model')
...
plot(SMI_rmod2a)
simple.eda(residuals(SMI_rmod2a))
shapiro.test(residuals(SMI_rmod2a))
```
...
hct_rmod2 <- lme4::lmer(data = dat_for_rehab,
                       hematocrit_percent ~ 
                         ...
                         (1|trial_number/individual_ID))
summary(hct_rmod2)
anova(hct_rmod1, hct_rmod2)
drop1(hct_rmod2)
```


try dropping either interaction:

```{r hct rehydration model 2ab}
...
hct_rmod3 <- lme4::lmer(data = dat_for_rehab,
...
hct_rmodels <- list(hct_rmod1, 
                   hct_rmod2, hct_rmod2a, hct_rmod2b, 
                   hct_rmod3,
                   hct_rmod_null)
...
The top model is model 1, the full model!
...
osml_rmod2 <- lme4::lmer(data = dat_for_rehab,
                       osmolality_mmol_kg_mean ~ 
                         ...
                         (1|trial_number/individual_ID))
summary(osml_rmod2)
anova(osml_rmod1, osml_rmod2)
drop1(osml_rmod2)
```

We do not need 3-way interaction.

try either 2-way-interaction:

```{r osml rehydration model 2ab}
...
single effect of rehab:

```{r osml rehydration model 3}
osml_rmod3 <- lme4::lmer(data = dat_for_rehab,
...
osml_rmodels <- list(osml_rmod1, 
                   osml_rmod2, osml_rmod2a, osml_rmod2b, 
                   osml_rmod3, 
                   osml_rmod_null)
...
rehydration_stats <- broom.mixed::tidy(osml_rmod_best1) %>%
  ...
  mutate(response = c(rep("Plasma Osmolality (mmol/kg)", 11),
                      rep("Hematocrit (%)", 11),
                      rep("Body Condition (g)", 7)))
write.csv(rehydration_stats, 
          "./results_statistics/rehydration_best_models.csv")

AICc_ranks_rehy <- osml_rAICc %>%
  rbind(hct_rAICc) %>%
  rbind(SMI_rAICc)
write.csv(AICc_ranks_rehy, 
          "./results_statistics/rehydration_model_rankings.csv")
```
...
ggplot(data = end_vals_SMI) +
  ...
  scale_color_brewer(palette = "Set2", name = "") +
  xlab("") + 
  annotate(geom = "text", x = 1, y = 13.8, label = "A", 
           size = 8) +
  annotate(geom = "text", x = 2, y = 12.3, label = "A", 
           size = 8) +
  annotate(geom = "text", x = 3, y = 14.3, label = "B", 
           size = 8) +
  annotate(geom = "text", x = 4, y = 14.5, label = "B", 
           size = 8) +
...
ggplot(data = end_vals_hct) +
  ...
  scale_color_brewer(palette = "Set2", name = "") +
  xlab("") + 
  annotate(geom = "text", x = 1, y = 42, label = "A", 
           size = 8) +
  annotate(geom = "text", x = 2, y = 45, label = "AB", 
           size = 8) +
  annotate(geom = "text", x = 3, y = 53, label = "B", 
           size = 8) +
  annotate(geom = "text", x = 4, y = 53, label = "B", 
           size = 8) +
  ylab("Final Hematocrit (%)") + 
  theme(text = element_text(color = "black", 
                            family = "sans", 
                            size = 22),
        axis.text = element_text(color = "black", 
...
dat_no_rehab2 <- dat_no_rehab %>%
  dplyr::filter(complete.cases(osmolality_mmol_kg_mean)) %>%
  dplyr::filter(individual_ID %in% dat_for_rehab2$individual_ID)
...
ggplot() + 
  geom_line(data = dat_2,
            aes(x = day_n,
                ...
                group = individual_ID),
            alpha = 0.2) +
  stat_smooth(data = dat_no_rehab_rm,
              aes(x = day_n,
                  y = osmolality_mmol_kg_mean, 
                  color = tmt,
                  group = tmt),
...
osml_CSU_fig
ggsave(filename = "CSU_osml_fig.jpeg",
       plot = osml_CSU_fig,
...
ggplot(data = end_vals_osml) +
  ...
  scale_color_brewer(palette = "Set2", name = "") +
  xlab("") + 
  annotate(geom = "text", x = 1, y = 445, label = "A", 
           size = 8) +
  annotate(geom = "text", x = 2, y = 420, label = "A", 
           size = 8) +
  annotate(geom = "text", x = 3, y = 465, label = "A", 
           size = 8) +
  annotate(geom = "text", x = 4, y = 430, label = "A", 
           size = 8) +
  ylab("Final Plasma Osmolality\n(mmol/kg)") + 
  theme(text = element_text(color = "black", 
                            family = "sans", 
                            size = 22),
        axis.text = element_text(color = "black", 
...
osml_end_boxplot
#ggsave(filename = "osml_end_boxplot.jpeg",
 #      plot = osml_end_boxplot,
  #     path = "./results_figures",
   #    device = "jpeg",
    #   dpi = 1200,
     #  width = 9, height = 6)
...
ggsave(filename = "CEWL_CSU_pres_fig.jpeg",
       plot = CEWL_CSU_pres_fig,
       path = "./results_figures",
       device = "jpeg",
       dpi = 1200,
       width = 8, height = 6)
...
TukeyHSD(aov(data = end_vals_CEWL, 
...
ggplot(data = end_vals_CEWL) +
  ...
  scale_color_brewer(palette = "Set2", name = "") +
  xlab("") + 
  ylab(bquote('Final CEWL (g/'*m^2*'h)')) + 
  annotate(geom = "text", x = 1, y = 61, label = "A", 
           size = 8) +
  annotate(geom = "text", x = 2, y = 48, label = "B", 
           size = 8) +
  annotate(geom = "text", x = 3, y = 49, label = "C", 
           size = 8) +
  annotate(geom = "text", x = 4, y = 39, label = "B", 
           size = 8) +
  theme(text = element_text(color = "black", 
                            family = "sans", 
                            size = 22),
...
#ggsave(filename = "CEWL_end_boxplot.jpeg",
 #      plot = CEWL_end_boxplot,
  #     path = "./results_figures",
   #    device = "jpeg",
    #   dpi = 1200,
     #  width = 8, height = 6)
...
ggsave(filename = "exp_ending_values_multi_fig.jpeg",
