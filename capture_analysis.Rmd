...
This data was collected June - August by Master's student Savannah Weaver, advisor Dr. Emily Taylor, and research assistants Robin Bedard, Tess McIntyre, and Taylor Van Rossum. Adult male *Sceloporus occidentalis* were caught across the Cal Poly campus and and in Poly Canyon. This R file analyzes hydrophysiological variation at the time of capture. Please refer to **doi:** for the published scientific journal article and full details.
...
Read-in and attach all data. Details described in the sub-sections below.

```{r}
full_dat <- read.csv("./data/mass_hct.csv", # filename
  ...
  dplyr::select(measurement_date,
                time_captured,
                time_processed,
                time_c_temp = temp_time, # will be different
                type, day, 
                individual_ID,
                mass_g, 
                blood_sample_eye, hemolyzed,
                hematocrit_percent, 
                osmolality_mmol_kg,
                CEWL_g_m2h,
                cloacal_temp_C
                ) %>%
  mutate(# put date and time together
                ...
                measurement_date = as.Date(measurement_date, 
                                          format = "%Y-%m-%d"),
                # correctly format time variables
                ...
                time_processed = (as.POSIXct(time_processed, 
                                              format = "%H:%M")),
                time_c_temp = (as.POSIXct(time_c_temp, 
                               format = "%Y-%m-%d %H:%M:%S")),
  ...
  #dplyr::filter(day == 0) %>%
  #dplyr::filter(complete.cases(individual_ID)) %>%
  #dplyr::select(-notes)
...
- measurement_date = date the measurements were taken, including capture day
- collection/capture time for each lizard
- type = whether measurements were during experiment (exp) or after rehydration (post-rehab). For this R script/analysis, I'm only going to use capture day data.
...
- any notes (will remove to keep dataframe clean)

```{r}
# load data, format, and select only capture data

# these two should be the same
unique(mass_hct_dat$capture_date)
unique(mass_hct_dat$measurement_date)
```
...
```{r}
osml_capture <- read.csv("./data/osml_means_clean.csv",
                         na.strings=c("","NA")
                         ) %>%
  # reformat data
...
```{r}
CEWL_capture <- read.csv("./data/CEWL_dat_all_clean.csv",
                         na.strings=c("","NA")
                         ) %>%
  # reformat data
  dplyr::mutate(# paste and format date-time variable
...
filter for capture only, join weather and SMI
...
```{r}
`%nin%` = Negate(`%in%`)
test <- (c(seq(201, 321, by = 1)))
lost <- test[test %nin% mass_hct_dat$individual_ID]
lost
```

We have capture data for all individual_ID's.

check that times are all within x of each other

check number of obs per individual
