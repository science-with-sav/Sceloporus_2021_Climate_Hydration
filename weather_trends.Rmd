...
Our analyses of baseline variation in CEWL and plasma osmolality yielded VERY different models for this current project/dataset compared to our last iteration of data collection (DOI:10.1655/Herpetologica-D-21-00030.1). In this Rmd, I investigate the weather trends across both studies to assess whether variation, or lack of, in weather may have led to the very different results we got.

I also want to know how close our treatment climates were to what the lizards experience in the wild, and I quantify that in this file.

The weather at capture that is used in the models of baseline variation are read-in in the "wrangling_general" file and analysed in the "analysis_capture" file.


# Load Data

First, get the sunrise and sunset times March - September.
source: [ESRL](https://gml.noaa.gov/grad/solcalc/table.php?lat=35.304442&lon=-120.65925&year=2021)

sunrise times:

```{r}
sunrises <- read.csv("./data/sunrise_SLO_2021.csv")
rise_March <- sunrises %>%
  dplyr::select(Day, time = Mar) %>%
  mutate(date = paste("2021", "03", Day, sep = "-"),
...
rise_July <- sunrises %>%
  ...
  mutate(date = paste("2021", "07", Day, sep = "-"),
         sunrise = as.POSIXct(paste(date, time, sep = " ")))
rise_August <- sunrises %>%
  dplyr::select(Day, time = Aug) %>%
  dplyr::filter(time != "") %>%
  mutate(date = paste("2021", "08", Day, sep = "-"),
         sunrise = as.POSIXct(paste(date, time, sep = " ")))
rise_September <- sunrises %>%
  dplyr::select(Day, time = Sep) %>%
  dplyr::filter(time != "") %>%
  mutate(date = paste("2021", "09", Day, sep = "-"),
         sunrise = as.POSIXct(paste(date, time, sep = " ")))

sunrises_formatted <- rise_March %>%
  rbind(rise_April) %>%
  rbind(rise_May) %>%
  rbind(rise_June) %>%
  rbind(rise_July) %>%
  rbind(rise_August) %>%
  rbind(rise_September) %>%
...
```{r}
sunsets <- read.csv("./data/sunset_SLO_2021.csv")
set_March <- sunsets %>%
  dplyr::select(Day, time = Mar) %>%
  mutate(date = paste("2021", "03", Day, sep = "-"),
...
set_August <- sunsets %>%
  ...
  mutate(date = paste("2021", "08", Day, sep = "-"),
         sunset = as.POSIXct(paste(date, time, sep = " ")))
set_September <- sunsets %>%
  dplyr::select(Day, time = Sep) %>%
  dplyr::filter(time != "") %>%
  mutate(date = paste("2021", "09", Day, sep = "-"),
         sunset = as.POSIXct(paste(date, time, sep = " ")))

sunsets_formatted <- set_March %>%
  rbind(set_April) %>%
  rbind(set_May) %>%
  rbind(set_June) %>%
  rbind(set_July) %>%
...
load in weather data from ITRC:
...
weather_dat_active_szn <- weather_dat_XL %>%
  ...
  left_join(sunrises_formatted, by = 'date') %>%
  left_join(sunsets_formatted, by = 'date') %>%
  group_by(date) %>%
...
summary(weather_dat_active_szn)
```
...
This is weather data specifically at the time of capture of each lizard in the study.

```{r}
cap_weather <- read_rds("./data/analysis_data_capture.RDS") %>%
...
The vertical lines added indicate the days we captured lizards for both studies conducted in 2021.
...
Create a figure to compare our experimental conditions to daytime conditions throughout the entire active season.
...
ggplot() +
  geom_tile(data = weather_dat_active_szn,
            ...
            height = 0.09,
            width = 0.6) +
  geom_point(data = cap_weather,
             aes(x = temp_C_interpol,
                 y = VPD_kPa_int),
             color = "black",
             size = 0.5
             ) +
  ...
  geom_point(data = tmts,
             aes(x = temp_C,
                 y = VPD_kPa), 
             shape = c(22,21,22,21), # same as my_shapes, but with no fill
             color = "black",
              size = 4,
              alpha = 1) +
  ...
  xlim(0,40) + ylim(-0.1,6) +
  #guides(shape = guide_legend(nrow = 4, byrow = TRUE),
   #      #fill = guide_legend(title.position = "left") # makes color ramp weird
    #     ) +
  annotate("text", label = "Cool Humid",#\n0.6 kPa",
            x = 26.8, y = 0.2, size = 2.6, color = "black") +
  annotate("text", label = "Hot Humid",#\n1.1 kPa",
            x = 37.4, y = 1.5, size = 2.6, color = "black") +
  annotate("text", label = "Cool Dry",#\n2.5 kPa",
            x = 22.2, y = 2.9, size = 2.6, color = "black") +
  annotate("text", label = "Hot Dry",#\n3.8 kPa",
            x = 37.3, y = 3.4, size = 2.6, color = "black") +
  # add custom legend title so I can put where I want
  annotate("text", label = "Relative Humidity (%)",
            x = 12, y = 6, size = 3, color = "black") +
  theme(text = element_text(color = "black", 
                            family = "sans", 
                            size = 12),
        axis.text = element_text(color = "black", 
...
# only need if legend outside plot
 ...
 #                                       ncol = 1, nrow = 1,
  #                                      common.legend = TRUE,
   #                                     legend = "bottom"
    #                                   )
# save
ggsave(filename = "exp_vs_weather_fig_VPD.pdf",
...
```{r}
anova(lm(data = weather_dat_daily, temp_C ~ date))
...
What level of relative humidity do these lizards typically experience?
...
Mean daily relative humidity during the CWL 2021 study was 73±9%, shockingly high!
