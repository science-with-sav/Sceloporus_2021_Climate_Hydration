...
CEWL_mod4.2a <- lme4::lmer(data = dat_no_rehab_rm,
                       CEWL_g_m2h_mean ~ 
                         cloacal_temp_C + 
                         day_n + 
                         day_n:VPD_kPa + 
                         ...
                         (1|trial_number))
CEWL_mod4.2b <- lme4::lmer(data = dat_no_rehab_rm,
                       CEWL_g_m2h_mean ~ 
                         cloacal_temp_C + 
                         day_n + 
                         ...
                         (1|trial_number))
#summary(CEWL_mod4.2)
VPDa <- data.frame(anova(CEWL_mod4.2a)) %>%
  mutate(model_order = "VPD listed before RH & temp",
         variable = row.names(.))
VPDb <- data.frame(anova(CEWL_mod4.2b)) %>%
  mutate(model_order = "VPD listed after RH & temp",
         variable = row.names(.)) %>%
  rbind(VPDa)
write.csv(VPDb, "./results_statistics/CEWL_VPD_SS_compare.csv")
#drop1(CEWL_mod4.2)
...
VPD_kPa                        1 1052.9  1052.9  36.9580
cloacal_temp_C                 1 3378.9  3378.9 118.6075
day_n                          1 1793.2  1793.2  62.9459
day_n:humidity_tmt             1 1376.9  1376.9  48.3314
day_n:temp_tmt                 1  383.4   383.4  13.4585
day_n:humidity_tmt:temp_tmt    1  211.8   211.8   7.4341
...
```{r CEWL exp model 4.3}
CEWL_mod4.3 <- lme4::lmer(data = dat_no_rehab_rm,
                       CEWL_g_m2h_mean ~ cloacal_temp_C + 
                         day_n + 
                         day_n:humidity_tmt + day_n:temp_tmt +
                         day_n:humidity_tmt:temp_tmt + 
                         (1|trial_number))
summary(CEWL_mod4.3)
anova(CEWL_mod4.2, CEWL_mod4.3)
drop1(CEWL_mod4.3)
car::vif(CEWL_mod4.3) # check collinearity 
```
