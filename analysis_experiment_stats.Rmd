...
anova(SMI_mod_best1, type = "1", ddf = "Kenward-Roger")
summary(try)

# test getting effect sizes
...
SMI_SS <- data.frame(anova(SMI_mod3)) %>%
  mutate(term = row.names(.),
         term = case_when(substr(term, (nchar(term)-5), nchar(term)) == "mp_tmt" ~ paste(term, "Cool", sep = ""),
                          substr(term, (nchar(term)-5), nchar(term)) != "mp_tmt" ~ term))

SMI_stats <- broom.mixed::tidy(SMI_mod_best1) %>%
  mutate(response = "Body Condition (g')") %>%
  left_join(SMI_SS, by = 'term')
```
...
osml_SS <- data.frame(anova(osml_mod4)) %>%
  mutate(term = row.names(.),
         term = case_when(substr(term, (nchar(term)-5), nchar(term)) == "mp_tmt" ~ paste(term, "Cool", sep = ""),
                          substr(term, (nchar(term)-5), nchar(term)) == "ty_tmt" ~ paste(term, "Dry", sep = ""),
                          substr(term, (nchar(term)-3), nchar(term)) != "_tmt" ~ term))

osml_stats <- broom.mixed::tidy(osml_mod_best1) %>%
  mutate(response = "Plasma Osmolality (mmol/kg)") %>%
  left_join(osml_SS, by = 'term')
```
...
dat_no_rehab_rm <- dat_no_rehab %>% # initiate new df
  ...
  mutate(delta_CEWL = CEWL_g_m2h_mean - start_CEWL) # calculate ∆CEWL after-before experiment
histogram(dat_no_rehab_rm$delta_CEWL, type = "count") # check ∆CEWL distribution
```
...
ggplot(dat_no_rehab) + 
  geom_point(aes(x = CEWL_g_m2h_mean,
                 y = cloacal_temp_C,
                 color = day_n))
summary(lm(data = dat_no_rehab, CEWL_g_m2h_mean ~ cloacal_temp_C*day_factor))
```
...
*I can no longer include body temp (doesn't make sense to...) for ∆CEWL, so check if there's any relationship of raw CEWL ~ body temp on day 8 measurements:

```{r}
body_temp_test_dat <- dat_no_rehab %>%
  dplyr::filter(complete.cases(CEWL_g_m2h_mean)) %>%
  dplyr::filter(day_n == 8)

CEWL_body_temp_mod <- lm(data = body_temp_test_dat, 
...
Next problem... 
It doesn't work to test any interaction effects... rank deficient even if I only try to use one interaction, so only test main effects.
...
There is incredibly high collinearity between VPD and humidity, so this model is invalid. Also, humidity has the opposite sign for the slope than expected, and sign change is a symptom of collinearity. So then, the best way to go about this will be to test each main effect individually, then compare the Rsq and RMSE of the models to figure out which one is best.
...
can I also incorporate 2-variable interaction models this way?

I will test VPD * temp and humidity * temp, but I can't do VPD * humidity because they are collinear and the model would be invalid.

```{r}
CEWL_mod1ac <- lme4::lmer(data = dat_no_rehab_rm,
                       delta_CEWL ~ VPD_kPa * temp_tmt + 
                         (1|trial_number))
CEWL_mod1bc <- lme4::lmer(data = dat_no_rehab_rm,
                       delta_CEWL ~ humidity_tmt * temp_tmt + 
                         (1|trial_number)) # interaction is significantly better
CEWL_mod1bc2 <- lme4::lmer(data = dat_no_rehab_rm,
                       delta_CEWL ~ humidity_tmt + temp_tmt + 
                         (1|trial_number))
anova(CEWL_mod1bc, CEWL_mod1bc2)
...
compare RMSE values between the models:

```{r}
# single-term models
VPD_RMSE <- sqrt(mean((residuals(CEWL_mod1a))^2))
humidity_RMSE <- sqrt(mean((residuals(CEWL_mod1b))^2))
temp_RMSE <- sqrt(mean((residuals(CEWL_mod1c))^2))

#2-term models
VPD_temp_RMSE <- sqrt(mean((residuals(CEWL_mod1ac))^2))
hum_temp_RMSE <- sqrt(mean((residuals(CEWL_mod1bc))^2))

compare_RMSE <- data.frame(RMSE = round(c(VPD_RMSE, humidity_RMSE, temp_RMSE,
                                    VPD_temp_RMSE, hum_temp_RMSE),
                                    digits = 2),
                           model = c("∆CEWL ~ VPD", 
                                     "∆CEWL ~ Humidity",
                                     "∆CEWL ~ Temperature",
                                     "∆CEWL ~ VPD*Temperature",
                                     "∆CEWL ~ Humidity*Temperature")) %>%
  arrange((RMSE))
compare_RMSE

# check AIC for VPD vs temp*RH tmt groups
anova(CEWL_mod1a, CEWL_mod1bc)

AIC <- data.frame(model = c("∆CEWL ~ VPD", 
                            "∆CEWL ~ Humidity*Temperature"),
                  AIC = c(935.78, 914.90))

compare_RMSE_AIC <- compare_RMSE %>%
  left_join(AIC, by = 'model')
```

I'm using RMSE, which is a measure of how good fit a model is to the data. Smaller RMSE values indicate better fit, but small differences in RMSE aren't very meaningful. Among models, whichever one has the lowest RMSE is the best explanation for the data.
...
plot(CEWL_mod1bc)
plot(CEWL_mod1ac)
simple.eda(residuals(CEWL_mod1a))
simple.eda(residuals(CEWL_mod1b))
simple.eda(residuals(CEWL_mod1c))
simple.eda(residuals(CEWL_mod1bc))
simple.eda(residuals(CEWL_mod1ac))
```
...
Save each model with p-values:

```{r best CEWL exp models}
CEWL_mod1ap <- lmerTest::lmer(data = dat_no_rehab_rm,
                       delta_CEWL ~ VPD_kPa + 
                         (1|trial_number))
df_a <- broom.mixed::tidy(CEWL_mod1ap) %>%
  mutate(model = "∆CEWL ~ VPD")

CEWL_mod1bp <- lmerTest::lmer(data = dat_no_rehab_rm,
                       delta_CEWL ~ humidity_tmt + 
                         (1|trial_number))
df_b <- broom.mixed::tidy(CEWL_mod1bp) %>%
  mutate(model = "∆CEWL ~ Humidity")

CEWL_mod1cp <- lmerTest::lmer(data = dat_no_rehab_rm,
                       delta_CEWL ~ temp_tmt +
                         (1|trial_number))
df_c <- broom.mixed::tidy(CEWL_mod1cp) %>%
  mutate(model = "∆CEWL ~ Temperature")

CEWL_mod1acp <- lmerTest::lmer(data = dat_no_rehab_rm,
                       delta_CEWL ~ VPD_kPa * temp_tmt + 
                         (1|trial_number))
df_ac <- broom.mixed::tidy(CEWL_mod1acp) %>%
  mutate(model = "∆CEWL ~ VPD*Temperature")

CEWL_mod1bcp <- lmerTest::lmer(data = dat_no_rehab_rm,
                       delta_CEWL ~ humidity_tmt * temp_tmt + 
                         (1|trial_number))
df_bc <- broom.mixed::tidy(CEWL_mod1bcp) %>%
  mutate(model = "∆CEWL ~ Humidity*Temperature")
...
CEWL_stats <- df_a %>%
  ...
  #rbind(df_ac) %>%
  rbind(df_bc) %>%
  left_join(compare_RMSE_AIC, by = 'model') %>%
  arrange(RMSE, model) %>%
  dplyr::select(AIC, RMSE, model, predictor = term, group, estimate,
                std.error, statistic, df, p.value)
write.csv(CEWL_stats, 
          "./results_statistics/exp_models_∆CEWL_only.csv")
```
...
I'll do this one a little differently, just using the mean ∆CEWL per tmt group, and dividing that by 8 to assume linear change and get per day rate change.

```{r}
CEWL_per_day <- dat_no_rehab_rm %>%
  group_by(tmt, VPD_kPa) %>%
  summarise(mean_delta_CEWL = mean(delta_CEWL),
            CEWL_per_day = mean_delta_CEWL/8)
```
...
```{r}
response_per_day <- CEWL_per_day %>%  
  left_join(osml_per_day, by = 'tmt') %>%
...
ggplot(data = end_vals_SMI) +
  ...
  geom_point(data = end_means, 
             ...
             shape = 23) +
  theme_classic() + 
  scale_shape_manual(values = c(21, 21, 22, 22), name = "") +
  scale_fill_brewer(palette = "Spectral", name = "") +
  scale_color_brewer(palette = "Spectral", name = "") +
  scale_y_continuous(limits = c(7,15),
                     breaks = c(seq(7,15, by = 2)),
                     labels = c(seq(7,15, by = 2))) +
  scale_x_discrete(labels = c("Hot Dry\n3.8",
                               "Hot Humid\n1.1",
                               "Cool Dry\n2.5",
                               "Cool Humid\n0.6")) + 
  xlab("") + 
...
# save
ggsave(filename = "experiment_CEWL_delta_boxplot.pdf",
       plot = CEWL_delta_boxplot,
       path = "./results_figures",
       device = "pdf",
       dpi = 600,
       units = "mm",
...
ggplot(data = dat_no_rehab_rm) +
  #geom_hline(yintercept = 0,
   #          linetype = "dashed",
    #         color = "darkgrey") +
  geom_abline(intercept = -7.9, slope = 11,
              linetype = "solid",
             color = "lightgrey") +
  geom_abline(intercept = 37.9, slope = -11,
              linetype = "dashed",
             color = "black") +
...
# save
ggsave(filename = "experiment_CEWL_delta_lines.pdf",
       plot = CEWL_delta_lines,
       path = "./results_figures",
       device = "pdf",
       dpi = 600,
       units = "mm",
